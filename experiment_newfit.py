...
def freqToPower(f,fo,q,b):
    return ((abs(((1-b+2j*b*(q*(f-fo)/fo)+4*(q*(f-fo)/fo)**2))/(1-4*(q*(f-fo)/fo)**2)))**2)*0.2
...
for index in range(0,len(filenames)):
	...
	# if index == 0:
	# 	plt.text(17.4,0.1,"Q = {}".format(np.absolute(popt[2])),fontdict = font)
	# 	plt.text(17.4,0.08,"F = {}".format(popt[1]),fontdict = font)
